...
export async function POST(req: NextRequest) {
  ...
  const { username, role, employeeId } = parsed.data;
  ...
  // Validasi pegawai eksis & belum punya akun (sesuai aturan autosuggest "tidak boleh memasukkan nama yang tidak ada dalam daftar")
  const emp = await db("employees").where({ id: employeeId }).whereNull("deleted_at").first();
  if (!emp) return NextResponse.json({ error: "Pegawai tidak ditemukan" }, { status: 400 });
  const empHasAccount = await db("users").where({ employee_id: employeeId }).whereNull("deleted_at").first();
  if (empHasAccount) return NextResponse.json({ error: "Pegawai sudah memiliki akun" }, { status: 409 });

  const roleRow = await db("roles").where({ name: role }).first();
  if (!roleRow) return NextResponse.json({ error: "Role tidak valid" }, { status: 400 });

  // Generate password acak (12 karakter, pasti memenuhi aturan: kapital, kecil, special)
  const plainPassword = crypto.randomBytes(8).toString("base64url").slice(0, 9) + "Aa1!";
  const argon2 = await import("argon2");
  const passwordHash = await argon2.hash(plainPassword);

  // Email & phone WAJIB dari data pegawai (server tidak trust client)
