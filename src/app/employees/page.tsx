...
import {
  Box, Button, Typography, Alert, Tooltip,
...
export default function EmployeesPage() {
  ...
  async function handleBulkDelete() {
    const ok = await confirm({
      title: "Hapus pegawai",
      message: `Hapus ${selected.length} pegawai terpilih? Tindakan ini tidak bisa dibatalkan.`,
      confirmLabel: "Hapus",
      danger: true,
    });
    if (!ok) return;
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: "join_date", headerName: "Tanggal Masuk", width: 130,
    ...
    {
      field: "actions", type: "actions", width: 130,
      getActions: ({ row }) => [
