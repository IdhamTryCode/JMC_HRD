...
const passwordSchema = z
  ...
  .refine((v) => !/\s/.test(v), "Tidak boleh ada spasi")
  .refine((v) => /[A-Z]/.test(v), "Harus ada huruf kapital")
  .refine((v) => /[a-z]/.test(v), "Harus ada huruf kecil")
  .refine((v) => /[^A-Za-z0-9]/.test(v), "Harus ada karakter spesial");
...
describe("passwordSchema", () => {
  it("password kuat lolos", () => {
    ...
    expect(passwordSchema.safeParse("Admin@2024x").success).toBe(true);
  });
  ...
  it("ada spasi ditolak", () => {
    expect(passwordSchema.safeParse("Rahasia 123!").success).toBe(false);
  });
