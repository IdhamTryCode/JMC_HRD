...
import {
  ...
  Typography, IconButton, Tooltip, Avatar, Skeleton,
} from "@mui/material";
...
const NAV: NavItem[] = [
  ...
  { label: "Pegawai",       href: "/employees",           icon: <PeopleIcon fontSize="small" />,       roles: ["admin_hrd", "manager_hrd"] },
  { label: "Presensi",      href: "/attendances",         icon: <AccessTimeIcon fontSize="small" />,   roles: ["admin_hrd", "manager_hrd"] },
  ...
  { label: "Log Aktivitas", href: "/logs",                icon: <HistoryIcon fontSize="small" />,      roles: ["superadmin"] },
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const visibleNav = session ? NAV.filter((n) => n.roles.includes(session.role)) : [];
    ...
    <Box sx={{ display: "flex", minHeight: "100vh", bgcolor: "background.default" }}>
      ...
      >
        ...
        <Box sx={{ flex: 1, overflow: "auto", py: 1.5, px: 1 }}>
          {!session ? (
            // Skeleton placeholder selama session belum loaded — mencegah flash
            <Box sx={{ px: 0.5 }}>
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <Box key={i} sx={{ display: "flex", alignItems: "center", gap: 1.5, py: 0.9, px: 1.5, mb: 0.25 }}>
                  <Skeleton variant="rounded" width={18} height={18} />
                  <Skeleton variant="text" width={`${60 + ((i * 13) % 30)}%`} height={16} />
                </Box>
              ))}
            </Box>
          ) : (
