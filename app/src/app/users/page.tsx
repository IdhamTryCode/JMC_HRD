...
export default function UsersPage() {
    ...
    try {
      if (editUser) {
        const res = await fetch(`/api/users/${editUser.id}`, {
          ...
          body: JSON.stringify({ username: formUsername, role: formRole, email: formEmail || null }),
        });
        const data = await res.json();
  ...
  async function handleToggleActive(row: UserRow) {
    const confirm = window.confirm(
      `${row.is_active ? "Nonaktifkan" : "Aktifkan"} user ${row.username}?`
    );
    if (!confirm) return;
  ...
  async function handleDelete(row: UserRow) {
    const confirm = window.confirm(`Hapus user ${row.username}? Tindakan ini tidak bisa dibatalkan.`);
    if (!confirm) return;
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  helperText="Ketik minimal 2 karakter — hanya pegawai tanpa akun"
                  size="small"
                />
              )}
            />
